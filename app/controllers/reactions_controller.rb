class ReactionsController < ApplicationController
  def new_user_reaction
      ...
      respond_to do |format|
          (@type == "comment") ? reaction_comment = Reaction.find_by(user_id: @user, comment_id: @comment.id) : reaction_article = Reaction.find_by(user_id: @user.id,publication_id: @publication.id)
